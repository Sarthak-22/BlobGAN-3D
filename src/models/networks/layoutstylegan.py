...
class LayoutStyleGANGenerator(nn.Module):
    ...
    def __post_init__(self):
        ...
        self.log_size = int(math.log(self.size, 2))                     # = 8
        self.log_size_in = int(math.log(self.size_in, 2))               # = 4
        self.c_in = self.channels[self.size_in]                         # = 768
        self.num_layers = (self.log_size - self.log_size_in) * 2 + 1    # = 9
        ...
        in_channel = self.channels[self.size_in]                         # = 768
        ...
        self.n_latent = (self.log_size - self.log_size_in + 1) * 2                             # = 10
    ...
    def forward(
            self,
            styles,
            input=None,               # feature grid obtained after convex combination : (4, 768, 16, 16)
        ...
        if not isinstance(styles, list):
            styles = [styles]               # [{'spatial style':(2,11,512), 'scores_pyramid': }] 
        ...
        if len(styles) < 2:
            ...
            if self.spatial_style:
                latent = styles * inject_index      # styles*10
    ...
    def forward(self, input, style):
        ...
        if spatial_style or not self.fused:
            if spatial_style:
                ...
                layout = style
                style = layout['spatial_style']                                                                      # (4, 11, 512)
                ...
                style = splat_features_from_scores(layout['scores_pyramid'][input.size(-1)], style, input.size(-1),  # (4, 512, 256, 256)  
                                                   channels_last=False)                        # style-based modulation - convex combination with scores  
                if self.demodulate:
