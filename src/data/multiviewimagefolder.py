...
class MultiViewImageFolderWithFilenames(Dataset):
    ...
    def __post_init__(self):
        ...
        self.camera_view1_path = os.path.join(self.basepath, self.cameras[0], self.split, self.split)   
        self.camera_view2_path = os.path.join(self.basepath, self.cameras[1], self.split, self.split)
    ...
    def __getitem__(self, index):
        ...
        print("img shape: {}".format(img_view1.size))

        img_view1_tformed = self.transform(img_view1)
        img_view2_tformed = self.transform(img_view2)  

        # Stacking the two images for the two views for the input image 
        output = [img_view1_tformed, img_view2_tformed]  
        print("img transformed 1 shape: {}".format(img_view1_tformed.shape))
        print("img transformed 2 shape: {}".format(img_view2_tformed.shape))

        # x, {'labels': y, 'filenames': self.imgs[i][0]}
        return output, {'labels': 'Nolabel', 'filenames': img_name}  
